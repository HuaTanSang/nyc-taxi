{{
    table_configuration(
        materialized='view',
        schema='staging',
        alias='stg_green_trips',
        tags=['staging', 'view', 'green_taxi_trips']
    )
}}

with source as (
    select *
    from {{ ref('raw_green_taxi') }}
)

select
    toUInt16OrNull(toString(VendorID)) as vendor_id,
    toDateTime64OrNull(toString(lpep_pickup_datetime), 3) as pickup_datetime,
    toDateTime64OrNull(toString(lpep_dropoff_datetime), 3) as dropoff_datetime,
    ...
    toFloat64OrNull(toString(ehail_fee)) as ehail_fee,
    ...
    toUInt8OrNull(toString(trip_type)) as trip_type_id,
